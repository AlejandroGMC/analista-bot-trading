...
def _escape_md2(text: str) -> str:
    ...
    Obligatorio para valores dinámicos (precios, símbolos, fechas).
...
def send_telegram_alert(token: str, chat_id: str,
    ...
    """
    Envía alerta de BOS a Telegram con MarkdownV2 correctamente escapado.

    FIX: Se reemplazó 'Markdown' (v1) por 'MarkdownV2' con escape explícito
    de todos los valores dinámicos. El Markdown v1 falla con emojis, guiones
    y puntos dentro de bloques de código inline.
    """
    # Valores dinámicos → siempre escapados
    sym          = _escape_md2(symbol)
    bos_type     = _escape_md2(bos["type"])
    close_price  = _escape_md2(f"{bos['close_price']:.4f}")
    broken_level = _escape_md2(f"{bos['broken_level']:.4f}")
    close_time   = _escape_md2(bos["close_time"].strftime("%Y-%m-%d %H:%M"))
    swing_time   = _escape_md2(bos["swing_time"].strftime("%Y-%m-%d %H:%M"))
    ...
    msg = (
        ...
        f"📌 *Símbolo:*  `{sym}`\n"
        f"🔖 *Tipo:* {bos_type}\n"
        f"💵 *Cierre:* `{close_price} USDT`\n"
        f"🎯 *Nivel roto:* `{broken_level} USDT`\n"
        f"🕐 *Cierre vela:* `{close_time} UTC`\n"
        f"📅 *Swing origen:* `{swing_time} UTC`\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"_Confirmado por cierre de vela 8H_"
    )
    ...
    payload = {
        "chat_id"    : chat_id,
        "text"       : msg,
        "parse_mode" : "MarkdownV2",   # ← FIX: v2 con escape explícito
    }
...
# Días activos: 0=Lunes … 6=Domingo
# Domingo(6), Lunes(0), Martes(1), Miércoles(2), Jueves(3)
ACTIVE_DAYS = {0, 1, 2, 3, 6}
ACTIVE_DAYS_NAMES = {0: "Lunes", 1: "Martes", 2: "Miércoles",
                     3: "Jueves", 6: "Domingo"}


def is_active_day() -> bool:
    """Retorna True si hoy es un día operativo (Dom–Jue)."""
    return datetime.now(timezone.utc).weekday() in ACTIVE_DAYS


def seconds_until_next_active_day() -> float:
    """
    Calcula los segundos hasta las 00:00 UTC del próximo día activo.
    Se usa para dormir el bot durante Viernes y Sábado.
    """
    from datetime import timedelta
    now = datetime.now(timezone.utc)
    for days_ahead in range(1, 8):
        candidate = (now.weekday() + days_ahead) % 7
        if candidate in ACTIVE_DAYS:
            wake_up = (now + timedelta(days=days_ahead)).replace(
...
def run_bot() -> None:
    ...
    Optimizado para PythonAnywhere:
      ...
      - Solo opera Dom / Lun / Mar / Mié / Jue (UTC)
        → Vie y Sáb: el bot duerme hasta el próximo día activo
    """
    logger.info("=" * 60)
    ...
    logger.info("  Días activos: Dom / Lun / Mar / Mié / Jue (UTC)")
    logger.info("=" * 60)
    ...
    while True:
        ...
        day_name    = ACTIVE_DAYS_NAMES.get(cycle_start.weekday(), None)

        # ── Verificación de día activo ──────────────────────────────────────
        if not is_active_day():
            inactive = ["Lunes","Martes","Miércoles","Jueves",
                        "Viernes","Sábado","Domingo"][cycle_start.weekday()]
            wait_secs = seconds_until_next_active_day()
            wait_hrs  = wait_secs / 3600
            logger.info(
                f"💤 Hoy es {inactive} — día NO operativo. "
                f"Bot en pausa {wait_hrs:.1f}h hasta el próximo día activo."
            )
...
# =============================================================================
# 9. SERVIDOR HTTP DUMMY (requerido por Render plan gratuito)
# =============================================================================
...
class _PingHandler(BaseHTTPRequestHandler):
    """
    Servidor mínimo que responde 200 OK a cualquier request.
    Solo existe para satisfacer el escaneo de puertos de Render.
    No afecta la lógica del bot.
    """
    def do_GET(self):
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b"Bot BOS 8H activo")
...
def start_http_server(port: int = 8080) -> None:
    ...
    logger.info(f"Servidor HTTP dummy escuchando en puerto {port} (Render keep-alive)")
...
if __name__ == "__main__":
    # Arranca el servidor HTTP en hilo separado (no bloquea el bot)
    http_thread = threading.Thread(target=start_http_server, daemon=True)
    http_thread.start()

    # Arranca el bot principal
    run_bot()
